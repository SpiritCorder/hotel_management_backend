...
const createNewVehicle = async (req, res, next) => {
    ...
    const query = `INSERT INTO vehicle 
        (name, type, vehicleCondition, seats, bags, doors, isAirConditioned, isAuto, fuelType, isDriverFree, fuelPolicy, pickupPolicy, price, overduePrice, paymentType, discount)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

    try {
        const [result] = await db.query(query, [name, type, condition, seats, bags, doors, isAirConditioned, isAuto, fuelType, isDriverFree, fuelPolicy, pickupPolicy, price, overduePrice, paymentType, discount]);
...
const updateVehicle = async (req, res, next) => {
    ...
    const query = `UPDATE vehicle SET name=?, type=?, vehicleCondition=?, seats=?, bags=?, doors=?, isAirConditioned=?, isAuto=?, fuelType=?, isDriverFree=?, fuelPolicy=?, pickupPolicy=?, price=?, overduePrice=?, paymentType=?, discount=? WHERE id = ?`;

    try {
        const [result] = await db.query(query, [name, type, condition, seats, bags, doors, isAirConditioned, isAuto, fuelType, isDriverFree, fuelPolicy, pickupPolicy, price, overduePrice, paymentType, discount, +vehicleId]);
